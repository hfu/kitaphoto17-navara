...
[DECISIONS.md](DECISIONS.md)の各エントリから参照される、詳しい調査ログの
アーカイブ。ここも追記専用— 既存の内容は書き換えない。DECISIONS.mdが
「今わかっている結論」の索引、こちらが「そこに至った経緯の生ログ」という
役割分担。
...
**この診断(crossOriginIsolatedが必要条件)はD11で誤りと判明した。**

## D8: Mapterhorn地形(terrain layer)はGitHub Pages上で `crossOriginIsolated` を条件にフィーチャー検出し、falseならスキップする
...
**結論(誤り、D11で訂正)**: raster-tile(平面画像表示)はワーカープール無しでも
動作するフォールバックがあるが、terrain(地形メッシュ生成)は現行の
`@navaramap/three@0.1.1`ではワーカープール必須で、起動失敗時のグレースフルな
フォールバックが実装されていないと判断した。GitHub PagesはCOOP/COEPヘッダを
設定できないため、**現行バージョンのNavara terrainはGitHub Pages上で原理的に
動作しない**、と結論づけたが、これはD11で誤りと判明した。

**対応(当時)**: `window.crossOriginIsolated`をフィーチャー検出し、`false`の
場合はterrainレイヤーの追加自体をスキップして、kitaphoto17を平面ラスターとして
表示する([main.ts](src/main.ts)参照)。
...
外す**のが妥当と判断した(この時点では真因がD11で判明する前だった)。
...
**追記(2026-09-05): Rustソースコードでの確認**。
`crates/navara_cesium3dtiles`と`crates/navara_parser/src/cesium3dtiles`の
全ソースを検索したが、`implicitTiling`/`.subtree`/`subdivisionScheme`という
3D Tiles 1.1 Implicit Tiling仕様のキーワードは一つも見つからなかった
(コード中の"subtree"は全て「タイルツリーの部分木」という一般的な木構造の
意味で、explicit/b3dm/glb/pnts/gltf_featuresには専用モジュールがあるのに
implicit tiling用のモジュールが存在しない)。これはバグではなく、
**Implicit Tilingのトラバーサルロジック自体が実装されていない**という
構造的な欠落であることが、実装コードから確定した。
...
原因は未特定(D17で結局raster-dem自体を使わなくなったため未調査のまま)。
...
## D12: PLATEAU建物・地形から「針」状のアーティファクトが出る(D16でraster-dem固有と判明)

D11の状態(kitaphoto17 + Mapterhorn地形 + 札幌PLATEAU建物を同時表示)を
hfuさんが実ブラウザで確認したところ、各建物・地形から空に向かって細く長い
...
**さらなる切り分け(2026-09-05、D16参照)**: navara.world公式の
`examples/terrain/quantized-mesh`(raster-demとは全く別のメッシュ構築
コードパス)をBraveで確認したところ、広い視野・拡大した近距離のどちらでも
針は一切発生しなかった。つまりD12は「Brave全般のWebGL問題」ではなく、
**「Navaraのraster-dem→地形メッシュ生成コードパス」×「Brave固有の何か」の
組み合わせ**で発生する、より狭い範囲の不具合と判明した(詳細はD16)。

**Opus提供の仮説(未検証)**: NavaraはRTE(relative-to-eye)座標を
`position_3d_high`/`position_3d_low`に分割し、`u_rteOne`(値1.0の意図的な
uniform、コンパイラによる減算畳み込みを防ぐための古典的防御)を使っている。
Chromium/ANGLE(macOSではMetalバックエンド)のシェーダ最適化がこの防御を
破ると、特定の頂点だけ精度崩壊で「針」になる、という説。ただしD16の
quantized-meshで非再現という結果は、座標変換ロジックがraster-demと
quantized-meshで共通なら矛盾するため、raster-dem側の高さ計算・メッシュ
生成に特有の処理を疑う方が筋が良いかもしれない。他にBraveが
`WEBGL_debug_renderer_info`を隠蔽することでNavara側のGPU tier判定
(`three/src/device.ts`、`three/src/quality.ts`)が誤り、低精度パスを
選んでいる可能性も指摘された。Chrome/Firefoxでの検証は行っていない。

**最終対応**: D17でraster-dem自体をやめ、quantized-meshに切り替えることで
この問題を回避した。

## D13: PLATEAU建物が真っ黒に見える件は、Navara側がglTFの色データを正しく読んでいないことが原因(D14で根本原因を特定・修正)
...
情報を正しく反映せずに描画しているためだと判断した(具体的な原因はD14)。
...
**検証結果(2026-09-05)**: hfuさんがBraveの実ブラウザで確認し、
建物に正しい色(赤い屋根、グレー系の壁)が表示されることを確認した。
修正確定。D12(針)は引き続きBraveで発生しており、この修正とは
独立した別問題であることも改めて確認された。
...
## D15: Mapterhornのデータソースを`terrain.reearth.land`に切り替え(D17でquantized-meshに再切替、経緯として記録)
...
フォーマット(Terrarium, tileSize 512)は同一。公式サンプルに合わせ
`maxZoom`を17→15に変更、attributionに"Re:Earth Terrain"を追加
(Mapterhornのattributionは維持)。
...
D12はデータソースの問題ではないことが改めて確認された(その後D16/D17で
raster-dem自体をやめる判断に至る)。
...
に切り替えた。`TERRARIUM_ELEVATION_DECODER`や`tileSize`は不要になり削除。
`requestVertexNormals: true`は設定、`requestWaterMask: true`は後述の
理由で見送った。
...
**検証結果(2026-09-05)**: hfuさんがBraveの実ブラウザで確認し、
kitaphoto17ドレープ・地形起伏・PLATEAU建物(正しい色)いずれも
正常にレンダリングされ、針は一切発生しないことを確認した。当初の
3つの懸念(地形クラッシュ・黒い建物・Brave針)すべてが解消。
